...
/*   Updated: 2024/11/05 14:00:53 by rabouzia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		while (j++ < (1080 * facteur))mlx_hook(ori->mlxwin, 3, 2L,
			...
			int key_press(int key, t_cube *cube)
			{
				...
				if (key == XK_w)
					move_player(cube, cos(cube->player.angle),
						sin(cube->player.angle));
				if (key == XK_s)
					move_player(cube, cos(-cube->player.angle),
						sin(-cube->player.angle));
				if (key == XK_a)
					move_player(cube, cos(cube->player.angle + M_PI / 2),
						sin(cube->player.angle + M_PI / 2));
				if (key == XK_d)
					move_player(cube, cos(cube->player.angle - M_PI / 2),
						sin(cube->player.angle - M_PI / 2));
				// if (key == XK_w)
			...
			void move_player(t_cube *cube, double move_x, double move_y)
			{
				...
				new_x = (int)(cube->player.plyr_x + move_x);
					// get the new x position
				// printf("x is %d\n",cube->player.plyr_x );
				new_y = (int)(cube->player.plyr_y + move_y);
				...
				if (cube->map.map2d[map_grid_y][map_grid_x] != '1'
					&& (cube->map.map2d[map_grid_y][cube->player.plyr_x
						/ TILE_SIZE] != '1'
			...
			int get_color(t_cube *cube, int flag) // get the color of the wall
			{
				...
				if (flag == 0)
				{
					if (cube->ray.ray_ngl > M_PI / 2 && cube->ray.ray_ngl < 3
						* (M_PI / 2))
			...
			int wall_hit(float x, float y, t_cube *cube) // check the wall hit
			{
				...
				if ((y_m >= cube->map.h_map || x_m >= cube->map.w_map))
					return (0);
				if (cube->map.map2d[y_m]
					&& x_m <= (int)strlen(cube->map.map2d[y_m]))
				...
				// get the horizontal intersection
			{
				...
				h_x = cube->player.plyr_x + (h_y - cube->player.plyr_y)
					/ tan(angl);
				...
				// get the vertical intersection
			{
				...
				v_y = cube->player.plyr_y + (v_x - cube->player.plyr_x)
					* tan(angl);
			...
			void cast_rays(t_cube *cube) // cast the rays
			{
				...
				ray = 0;
				cube->ray.ray_ngl = cube->player.angle - (cube->player.fov_rd
						/ 2);
				// the start angle
				while (ray < S_W)
				// loop for the rays
